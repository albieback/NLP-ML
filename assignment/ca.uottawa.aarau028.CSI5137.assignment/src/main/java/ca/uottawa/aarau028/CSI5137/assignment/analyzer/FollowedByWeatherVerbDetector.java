...
public class FollowedByWeatherVerbDetector extends JCasAnnotator_ImplBase {
		...
		try {
			...
			for (Sentence sent : JCasUtil.select(aJCas, Sentence.class)) {
				for (It it : JCasUtil.selectCovered(It.class, sent)) {
					it.setFollowedByWeatherVerb(false);
					for (Token token : JCasUtil.selectFollowing(Token.class, it, 1)) {
						...
						List<Synset> senses = v.getSenses();
							...
							for (Synset s : senses) {
								if (s.getLexFileName().contentEquals("verb.weather")) it.setFollowedByWeatherVerb(true);
