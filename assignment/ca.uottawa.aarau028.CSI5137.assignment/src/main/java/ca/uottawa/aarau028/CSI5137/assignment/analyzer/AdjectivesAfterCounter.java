...
public class AdjectivesAfterCounter extends JCasAnnotator_ImplBase {
		...
		for (Sentence s : JCasUtil.select(aJCas, Sentence.class)) {
			for (It it : JCasUtil.selectCovered(It.class, s)) {
				...
				for (Token token : JCasUtil.selectFollowing(Token.class, it,
					...
					if (token.getPos().getCoarseValue().equals("ADJ"))
						adjectives++;
					;
				}
				it.setAdjectivesAfterCount(adjectives);
				it.setAdjectivesAfter(adjectives>0);
